...
def scale_init(shape, dtype='float32'):
    sc = np.linspace(0.5, 1.6, shape[0])
...
class GaussScaler(Layer):
    ...
    def build(self, input_shape):
        ...
        self.cov_scaler = self.add_weight(shape=(input_dim*self.filters,),
...
#        cov_scaled = K.batch_dot(self.cov_scaler,self.cov, axes=[1,2])
        ...
        sums = K.sum(masks, axis=(0, 1), keepdims=True)
        #ums = sums * sums
        #print("sums shape: ", sums.shape)
        #print(sums)
        # Sum normalisation
        gain = K.constant(np.sqrt(2.0),dtype='float32')
